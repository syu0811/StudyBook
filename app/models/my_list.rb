class MyList < ApplicationRecord
  ...
  scope :trend, -> { MyList.left_joins(:subscribe_my_lists).select('my_lists.id, my_lists.user_id, my_lists.category_id, my_lists.title, my_lists.description, my_lists.created_at, COUNT(*) as count').where("subscribe_my_lists.id IS NOT NULL").group("my_lists.id, my_lists.user_id, my_lists.category_id, my_lists.title, my_lists.description, my_lists.created_at").order("count DESC", "my_lists.id") }
