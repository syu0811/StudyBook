      ...
      concat(
        content_tag(:li) do
          concat(content_tag(:div, class: "list") do
            ...
            concat(link_to('ノート', user_notes_path(params.permit(:category, :order).merge(directory_path: nil)), remote: true))
          end)
          ...
          concat(
            content_tag(:li) do
              if tree[:children]
                concat(content_tag(:div, class: "list") do
                  concat(image_pack_tag('folder_icon.png', onClick: "toggleDirectory(event)"))
                  concat(link_to(tree[:name], user_notes_path(params.permit(:category, :order).merge(directory_path: tree[:path])), remote: true))
                end)
              else
